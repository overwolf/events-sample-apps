// this a subset of the features that Ancestors Legacy events provides - however,
...
function gameLaunched(gameInfoResult) {
  ...
  if (Math.floor(gameInfoResult.gameInfo.id/10) != 21236) {
    return false;
  }

  console.log("Ancestors Legacy Launched");
  return true;
...
function gameRunning(gameInfo) {
  ...
  // NOTE: we divide by 10 to get the game class id without it's sequence number
  if (Math.floor(gameInfo.id/10) != 21236) {
    return false;
  }

  console.log("Ancestors Legacy running");
  return true;
