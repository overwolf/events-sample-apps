// this a subset of the features that Hunt Showdown events provides - however,
...
function gameLaunched(gameInfoResult) {
  ...
  if (Math.floor(gameInfoResult.gameInfo.id/10) != 21328) {
    return false;
  }

  console.log("Hunt Showdown Launched");
  return true;
...
function gameRunning(gameInfo) {
  ...
  // NOTE: we divide by 10 to get the game class id without it's sequence number
  if (Math.floor(gameInfo.id/10) != 21328) {
    return false;
  }

  console.log("Hunt Showdown running");
  return true;
