// this a subset of the features that Eternal Return events provides - however,
...
function gameLaunched(gameInfoResult) {
  ...
  if (Math.floor(gameInfoResult.gameInfo.id/10) != 21672) {
    return false;
  }

  console.log("Eternal Return Launched");
  return true;
...
function gameRunning(gameInfo) {
  ...
  // NOTE: we divide by 10 to get the game class id without it's sequence number
  if (Math.floor(gameInfo.id/10) != 21672) {
    return false;
  }

  console.log("Eternal Return running");
  return true;
