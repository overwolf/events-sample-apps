// this a subset of the features that Final Fantasy XIV events provides - however,
...
function gameLaunched(gameInfoResult) {
  ...
  if (Math.floor(gameInfoResult.gameInfo.id/10) != 6350) {
    return false;
  }

  console.log("Final Fantasy XIV: Launched");
  return true;
...
function gameRunning(gameInfo) {
  ...
  // NOTE: we divide by 10 to get the game class id without it's sequence number
  if (Math.floor(gameInfo.id/10) != 6350) {
    return false;
  }

  console.log("Final Fantasy XIV running");
  return true;
