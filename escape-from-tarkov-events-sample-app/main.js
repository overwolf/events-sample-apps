// this a subset of the features that Escape From Tarkov events provides - however,
...
function gameLaunched(gameInfoResult) {
  ...
  if (Math.floor(gameInfoResult.gameInfo.id/10) != 21598) {
    return false;
  }

  console.log("Escape From Tarkov Launched");
  return true;
...
function gameRunning(gameInfo) {
  ...
  // NOTE: we divide by 10 to get the game class id without it's sequence number
  if (Math.floor(gameInfo.id/10) != 21598) {
    return false;
  }

  console.log("Escape From Tarkov running");
  return true;
